...
- **Fix**: pending the isolation test (below). App-side there is nothing to fix. NOTE: occurrence #3 self-resolved with ZERO action on our side - final proof the app/server were never involved. The isolation test is only meaningful DURING a failure window - next time it breaks, run incognito FIRST, before reporting.
